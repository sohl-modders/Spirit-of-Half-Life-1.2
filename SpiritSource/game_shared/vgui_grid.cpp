...
{
	AssertCheck(col >= 0 && col < m_xCols && row >= 0 && row < m_yRows,
	            "CGrid::GetEntryBox: invalid location specified");
...
void CGrid::paint()
{
	...
	for (int row = 0; row < m_yRows; row++)
	{
		...
		{
			drawSetColor(cell->m_UnderlineColor[0], cell->m_UnderlineColor[1], cell->m_UnderlineColor[2],
			             cell->m_UnderlineColor[3]);
