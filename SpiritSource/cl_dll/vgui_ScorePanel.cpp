...
void ScorePanel::SortTeams()
{
	...
	for (i = 1; i <= m_iNumTeams; i++)
	{
		...
		if (g_TeamInfo[i].players > 0)
		{
			...
			// use the average ping of all the players in the team as the teams ping
	...
	while (1)
	{
		...
		// set the already_drawn to be TRUE, so this team won't get sorted again
		m_iRows++;
			...
			if (m_iIsATeam[row])
			{
				...
				case COLUMN_NAME:
					...
					if (m_iIsATeam[row] == TEAM_YES)
					{
						...
						{
							sprintf(sz2, "(%d %s)", team_info->players,
							        CHudTextMessage::BufferedLocaliseTextString("#Player"));
						}
						else
						{
							sprintf(sz2, "(%d %s)", team_info->players,
							        CHudTextMessage::BufferedLocaliseTextString("#Player_plural"));
						}
					...
					if (bShowClass)
					{
						...
						if (bNoClass)
							...
							sprintf(sz, "%s", CHudTextMessage::BufferedLocaliseTextString(
								        sLocalisedClasses[g_PlayerExtraInfo[m_iSortedRows[row]].playerclass]));
					}
					else
					{
						strcpy(sz, "");
