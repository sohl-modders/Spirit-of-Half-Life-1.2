...
====================
*/
void CStudioModelRenderer::StudioCalcBoneAdj(float dadt, float* adj, const byte* pcontroller1, const byte* pcontroller2,
                                             byte mouthopen)
{
	int i, j;
	float value;
	...
	for (j = 0; j < m_pStudioHeader->numbonecontrollers; j++)
	{
		...
		if (i <= 3)
		{
			...
			if (pbonecontroller[j].type & STUDIO_RLOOP)
			{
				...
				{
					value = ((pcontroller1[i] * dadt + (pcontroller2[i]) * (1.0 - dadt))) * (360.0 / 256.0) +
		...
		if (panim->offset[j] != 0)
		{
			...
			*/
			...
			else
			{
				...
				if (panimvalue->num.total <= k + 1)
				{
					pos[j] += (panimvalue[panimvalue->num.valid].value * (1.0 - s) + s * panimvalue[panimvalue->num.
						valid + 2].value) * pbone->scale[j];
				}
				else
				{
...
void CStudioModelRenderer::StudioSetUpTransform(int trivial_accept)
{
	...
	if (m_pCurrentEntity->curstate.movetype == MOVETYPE_STEP)
	{
		...
		{
			f = (m_clTime - m_pCurrentEntity->curstate.animtime) / (m_pCurrentEntity->curstate.animtime -
...
float CStudioModelRenderer::StudioEstimateFrame(mstudioseqdesc_t* pseqdesc)
{
	...
	if (m_fDoInterp)
	{
		...
		{
			dfdt = (m_clTime - m_pCurrentEntity->curstate.animtime) * m_pCurrentEntity->curstate.framerate * pseqdesc->
				fps;
		}
	}
	else
...
void CStudioModelRenderer::StudioSetupBones(void)
{
	...
	pseqdesc = (mstudioseqdesc_t*)((byte*)m_pStudioHeader + m_pStudioHeader->seqindex) + m_pCurrentEntity->curstate.
		sequence;
	...
	*/
	...
	if (pseqdesc->numblends > 1)
	{
		...
		dadt = StudioEstimateInterpolant();
		s = (m_pCurrentEntity->curstate.blending[0] * dadt + m_pCurrentEntity->latched.prevblending[0] * (1.0 - dadt)) /
			255.0;
		...
		if (pseqdesc->numblends == 4)
		{
			...
			s = (m_pCurrentEntity->curstate.blending[1] * dadt + m_pCurrentEntity->latched.prevblending[1] * (1.0 - dadt
			)) / 255.0;
		...
		(m_pCurrentEntity->latched.prevsequence < m_pStudioHeader->numseq))
	{
		...
		pseqdesc = (mstudioseqdesc_t*)((byte*)m_pStudioHeader + m_pStudioHeader->seqindex) + m_pCurrentEntity->latched.
			prevsequence;
		panim = StudioGetAnim(m_pRenderModel, pseqdesc);
		// clip prevframe
	...
	if (m_pPlayerInfo && m_pPlayerInfo->gaitsequence != 0)
	{
		...
		pseqdesc = (mstudioseqdesc_t*)((byte*)m_pStudioHeader + m_pStudioHeader->seqindex) + m_pPlayerInfo->
			gaitsequence;
...
int CStudioModelRenderer::StudioDrawModel(int flags)
{
	...
	if (m_pCurrentEntity->curstate.renderfx == kRenderFxDeadPlayer)
	{
		...
		//cl.frames[cl.parsecount & CL_UPDATE_MASK].playerstate[m_pCurrentEntity->curstate.renderamt-1];
...
int CStudioModelRenderer::StudioDrawPlayer(int flags, entity_state_t* pplayer)
{
	...
	if (flags & STUDIO_RENDER)
	{
		...
		if (!(m_pCvarDeveloper->value == 0 && gEngfuncs.GetMaxClients() == 1) && (m_pRenderModel == m_pCurrentEntity->
			model))
		{
