...
CSchemeManager::CSchemeManager(int xRes, int yRes)
{
	...
	while (strlen(token) > 0 && (currentScheme < numTmpSchemes))
	{
		...
		if (!stricmp(paramName, "SchemeName"))
		{
			...
			hasFgColor = hasBgColor = hasArmedFgColor = hasArmedBgColor = hasMouseDownFgColor = hasMouseDownBgColor =
				false;
...
buildDefaultFont:
	...
	if (currentScheme < 0)
	{
		...
		tmpSchemes[0].armedFgColor[0] = tmpSchemes[0].armedFgColor[1] = tmpSchemes[0].armedFgColor[2] = tmpSchemes[0].
			armedFgColor[3] = 255;
		tmpSchemes[0].mousedownFgColor[0] = tmpSchemes[0].mousedownFgColor[1] = tmpSchemes[0].mousedownFgColor[2] =
			tmpSchemes[0].mousedownFgColor[3] = 255;
	}
