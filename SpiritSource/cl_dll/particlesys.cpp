...
bool ParticleSystem::UpdateParticle(particle* part, float frametime)
{
	...
	else
	{
		...
		if (part->pType->m_bBouncing)
		{
			...
			pmtrace_t* tr = gEngfuncs.PM_TraceLine(part->origin, vecTarget, PM_TRACELINE_PHYSENTSONLY, 2 /*point hull*/,
			                                       -1);
			if (tr->fraction < 1)
			{
				...
				VectorMA(part->velocity, -bounceforce * (newspeed + part->pType->m_Bounce.GetInstance()),
				         tr->plane.normal, part->velocity);
			}
		}
	}
