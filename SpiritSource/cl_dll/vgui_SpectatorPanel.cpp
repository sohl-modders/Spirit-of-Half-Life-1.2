...
void SpectatorPanel::Initialize()
{
	...
	m_OptionButton = new DropDownButton(CHudTextMessage::BufferedLocaliseTextString("#SPECT_OPTIONS"), XRES(15),
	                                    YRES(6), XRES(OPTIONS_BUTTON_X), YRES(20), false, false);
	...
	m_CamButton = new DropDownButton(CHudTextMessage::BufferedLocaliseTextString("#CAM_OPTIONS"),
	                                 ScreenWidth - (XRES(CAMOPTIONS_BUTTON_X) + 15), YRES(6), XRES(CAMOPTIONS_BUTTON_X),
	                                 YRES(20), false, false);
	...
	m_PrevPlayerButton = new CImageButton("arrowleft", XRES(15 + OPTIONS_BUTTON_X + 15), YRES(6), XRES(24), YRES(20),
	                                      false, false);
	...
	m_NextPlayerButton = new CImageButton("arrowright",
	                                      (ScreenWidth - (XRES(CAMOPTIONS_BUTTON_X) + 15)) - XRES(24 + 15), YRES(6),
	                                      XRES(24), YRES(20), false, false);
...
void SpectatorPanel::ShowMenu(bool isVisible)
{
	...
	if (!isVisible)
	{
		...
		if (m_menuVisible && this->isVisible())
		{
			...
			_snprintf(string, sizeof(string) - 1, "%c%s", HUD_PRINTCENTER,
...
void SpectatorPanel::Update()
{
	...
	m_TimerImage->setBounds(xPos, YRES(SEPERATOR_HEIGHT) + iTextHeight, XRES(SEPERATOR_WIDTH*2 + 1),
	                        YRES(SEPERATOR_HEIGHT + 1));

	m_CurrentTime->setBounds(xPos + XRES(SEPERATOR_WIDTH*2 + 1), YRES(SEPERATOR_HEIGHT) + iTextHeight, iTimeWidth,
	                         iTimeHeight);
	...
	for (j = 0; j < TEAM_NUMBER; j++)
	{
		...
		m_TeamScores[j]->setBounds(
			ScreenWidth - (iTextWidth + XRES(2*SEPERATOR_WIDTH+2*SEPERATOR_WIDTH/2+offset) + iwidth),
			YRES(SEPERATOR_HEIGHT) + (iheight * j), iwidth, iheight);
	}
}
