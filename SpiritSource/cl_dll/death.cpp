...
int CHudDeathNotice::Draw(float flTime)
{
	...
	for (int i = 0; i < MAX_DEATHNOTICES; i++)
	{
		...
		rgDeathNoticeList[i].flDisplayTime = V_min(rgDeathNoticeList[i].flDisplayTime,
		                                           gHUD.m_flTime + DEATHNOTICE_DISPLAY_TIME);
...
int CHudDeathNotice::MsgFunc_DeathMsg(const char* pszName, int iSize, void* pbuf)
{
	...
	else
	{
		...
		if (killedwith && *killedwith && (*killedwith > 13) && strcmp(killedwith, "d_world") && !rgDeathNoticeList[i].
			iTeamKill)
		{
			ConsolePrint(" with ");
