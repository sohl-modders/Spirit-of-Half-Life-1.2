...
//================================================================
// CreateSubMenu
CCommandMenu* TeamFortressViewport::CreateSubMenu(CommandButton* pButton, CCommandMenu* pParentMenu, int iYOffset,
                                                  int iXOffset)
{
	int iXPos = 0;
	int iYPos = 0;
                                                                               ...
                                                                               m_SchemeManager(wide, tall)
{
	...
	m_SpectatorOptionsMenu = CreateCommandMenu("spectatormenu.txt", 1, PANEL_HEIGHT, true, CMENU_SIZE_X,
	                                           BUTTON_SIZE_Y / 2, 0); // above bottom bar, flat design
	m_SpectatorCameraMenu = CreateCommandMenu("spectcammenu.txt", 1, PANEL_HEIGHT, true, XRES(200), BUTTON_SIZE_Y / 2,
	                                          ScreenWidth - (XRES(200) + 15)); // above bottom bar, flat design

	m_PlayerMenu = m_iNumMenus;
	m_iNumMenus++;

	float flLabelSize = ((ScreenWidth - (XRES(CAMOPTIONS_BUTTON_X) + 15)) - XRES(24 + 15)) - XRES(
		(15 + OPTIONS_BUTTON_X + 15) + 38);

	m_pCommandMenus[m_PlayerMenu] = new CCommandMenu(NULL, 1,
	                                                 XRES(( 15 + OPTIONS_BUTTON_X + 15 ) + 31),PANEL_HEIGHT,
	                                                 flLabelSize, 300);
...
//-----------------------------------------------------------------------------
int TeamFortressViewport::CreateCommandMenu(const char* menuFile, int direction, int yOffset, bool flatDesign,
	...
	try
	{
		...
		while ((strlen(token) > 0) && (m_iNumMenus < MAX_MENUS))
		{
			...
			while (token[0] != '}' && (strlen(token) > 0))
			{
				...
				{
					pButton = new ToggleCommandButton(cCommand, cText, xOffset, iButtonY, flButtonSizeX, flButtonSizeY,
					                                  flatDesign);
				}
				else if (direction == 1)
				{
				...
				{
					// normal button
					pButton = new CommandButton(iPlayerClass, cText, xOffset, iButtonY, flButtonSizeX, flButtonSizeY,
					                            flatDesign);
				}
...
CommandButton* TeamFortressViewport::CreateCustomButton(char* pButtonText, char* pButtonName, int iYOffset)
{
	...
	if (!strcmp(pButtonName, "!CHANGETEAM"))
	{
		...
		// Auto Assign button
		m_pTeamButtons[4] = new TeamButton(5, gHUD.m_TextMessage.BufferedLocaliseTextString("#Team_AutoAssign"), 0,
		                                   BUTTON_SIZE_Y, CMENU_SIZE_X, BUTTON_SIZE_Y);
		m_pTeamButtons[4]->addActionSignal(new CMenuHandler_StringCommand("jointeam 5"));
		pMenu->AddButton(m_pTeamButtons[4]);

		// Spectate button
		m_pTeamButtons[5] = new SpectateButton(CHudTextMessage::BufferedLocaliseTextString("#Menu_Spectate"), 0,
		                                       BUTTON_SIZE_Y, CMENU_SIZE_X, BUTTON_SIZE_Y, false);
...
void TeamFortressViewport::UpdateSpectatorPanel()
{
	...
	if (g_iUser1 && gHUD.m_pCvarDraw->value && !gHUD.m_iIntermission) // don't draw in dev_overview mode
	{
		...
		if (!m_pSpectatorPanel->isVisible())
		{
			...
			_snprintf(tempString, sizeof(tempString) - 1, "%c%s", HUD_PRINTCENTER,
			          CHudTextMessage::BufferedLocaliseTextString("#Spec_Duck"));
		...
		if (gEngfuncs.IsSpectateOnly())
		{
			...
			_snprintf(szText, 63, "%s: %d", CHudTextMessage::BufferedLocaliseTextString("#Spectators"),
...
CMenuPanel* TeamFortressViewport::CreateTextWindow(int iTextToShow)
{
	...
	CMenuPanel* pMOTDPanel = CMessageWindowPanel_Create(cText, cTitle, g_iPlayerClass == PC_UNDEFINED, false, 0, 0,
...
void TeamFortressViewport::UpdateCursorState()
{
	...
	if (m_pSpectatorPanel->m_menuVisible || m_pCurrentMenu || m_pTeamMenu->isVisible() || m_pServerBrowser->isVisible()
		|| GetClientVoiceMgr()->IsInSquelchMode() || (g_pGameUI && g_pGameUI->IsGameUIVisible()))
	{
...
void TeamFortressViewport::GetAllPlayersInfo(void)
{
	for (int i = 1; i < MAX_PLAYERS; i++)
	{
		...
		// !!!HACK: this should be initialized elsewhere... maybe gotten from the engine
	}
}
