...
void CTriggerRelay::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (pev->spawnflags & SF_RELAY_DEBUG)
	{
		ALERT(at_debug, "DEBUG: trigger_relay \"%s\" was sent %s",STRING(pev->targetname),
...
void CEnvCustomize::Affect(CBaseEntity* pTarget, USE_TYPE useType)
{
	...
	if (pev->spawnflags & SF_CUSTOM_DEBUG)
		ALERT(at_debug, "DEBUG: env_customize \"%s\" affects %s \"%s\": [", STRING(pev->targetname),
...
void CTriggerMotion::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (m_iszPosition)
	{
		...
		case 0:
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Set origin from %f %f %f ", pTarget->pev->origin.x, pTarget->pev->origin.y,
	...
	if (m_iszAngles)
	{
		...
		case 0:
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Set angles from %f %f %f ", pTarget->pev->angles.x, pTarget->pev->angles.y,
		...
		case 1:
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Rotate angles from %f %f %f ", pTarget->pev->angles.x, pTarget->pev->angles.y,
				      pTarget->pev->angles.z);
	...
	if (m_iszVelocity)
	{
		...
		case 0:
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Set velocity from %f %f %f ", pTarget->pev->velocity.x,
		...
		case 2:
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Rotate velocity from %f %f %f ", pTarget->pev->velocity.x,
				      pTarget->pev->velocity.y, pTarget->pev->velocity.z);
	...
	case 0:
		...
		if (pev->spawnflags & SF_MOTION_DEBUG)
			ALERT(at_debug, "DEBUG: Set avelocity from %f %f %f ", pTarget->pev->avelocity.x, pTarget->pev->avelocity.y,
...
void CMotionThread::Think(void)
{
	...
	if (m_iszPosition)
	{
		...
		case 0: // set position
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Set origin from %f %f %f ", m_hTarget->pev->origin.x, m_hTarget->pev->origin.y,
		...
		case 1: // offset position (= fake velocity)
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Offset origin from %f %f %f ", m_hTarget->pev->origin.x,
		...
		case 2: // set velocity
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Set velocity from %f %f %f ", m_hTarget->pev->velocity.x,
		...
		case 3: // accelerate
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Accelerate from %f %f %f ", m_hTarget->pev->velocity.x,
				      m_hTarget->pev->velocity.y, m_hTarget->pev->velocity.z);
			UTIL_SetVelocity(
				m_hTarget,
		...
		case 4: // follow position
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Set velocity (path) from %f %f %f ", m_hTarget->pev->velocity.x,
				      m_hTarget->pev->velocity.y, m_hTarget->pev->velocity.z);
			UTIL_SetVelocity(
	...
	if (m_iszFacing)
	{
		...
		case 0: // set angles
			...
			{
				if (pev->spawnflags & SF_MOTION_DEBUG)
					ALERT(at_debug, "DEBUG: Set angles from %f %f %f ", m_hTarget->pev->angles.x,
		...
		case 1: // offset angles (= fake avelocity)
			...
			{
				if (pev->spawnflags & SF_MOTION_DEBUG)
					ALERT(at_debug, "DEBUG: Offset angles from %f %f %f ", m_hTarget->pev->angles.x,
					      m_hTarget->pev->angles.y, m_hTarget->pev->angles.z);
		...
		case 2: // offset angles (= fake avelocity)
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Rotate angles from %f %f %f ", m_hTarget->pev->angles.x,
				      m_hTarget->pev->angles.y, m_hTarget->pev->angles.z);
		...
		case 3: // set avelocity
			...
			if (pev->spawnflags & SF_MOTION_DEBUG)
				ALERT(at_debug, "DEBUG: Set avelocity from %f %f %f ", m_hTarget->pev->avelocity.x,
